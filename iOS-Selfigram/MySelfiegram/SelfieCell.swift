...
class SelfieCell: UITableViewCell {
                ...
                query.findObjectsInBackground(block: { (users, error) -> Void in
                    ...
                    if let users = users as? [PFUser]{
                        for user in users {
                            ...
                            // objectId is a great way to compare if two objects
                    ...
                    if success {
                        ...
                        if let activityQuery = Activity.query(){
                            ...
                            activityQuery.findObjectsInBackground(block: { (activities, error) -> Void in
                                ...
                                // but this is code is checking for one or multiple instances
    ...
    func tapAnimation() {
        ...
        doubleTapImageView.isHidden = false;

        UIView.animate(withDuration: 1.0, delay: 0, options: [], animations: { () -> Void in
            
            self.doubleTapImageView.transform = CGAffineTransform(scaleX: 3, y: 3)
            
            
        }) { (success) -> Void in
            
            self.doubleTapImageView.transform = CGAffineTransform(scaleX: 0, y: 0)
            self.doubleTapImageView.isHidden = true
        }
